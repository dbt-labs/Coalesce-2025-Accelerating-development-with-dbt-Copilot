...
final as (
    select 
        ...
        case 
            when datediff(year, order_summary.last_order_date, current_date) < 1 then 'current'
            else 'inactive'
        end as customer_status
    from customers
    inner join order_summary on customers.customer_id = order_summary.customer_id
)
