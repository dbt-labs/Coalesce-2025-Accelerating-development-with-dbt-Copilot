...
4. Add a rule that turns the three general review steps from Exercise 2 into something Wizard does
...
recognizes a matching task, or invoked explicitly in a prompt.
...
2. Give it a `description` in the frontmatter that says when Wizard should invoke it (e.g.
   "Use when building a new dbt mart model.") - this is what Wizard actually matches against to
...
actually belongs in so you can deprecate `team_notes.md`.
