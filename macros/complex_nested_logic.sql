{% macro complex_nested_logic(input_list) %}
    {%- set result = [] -%}
    {% for item in input_list %}  {# Corrected to iterate over input_list #}
        {% if item is not none %}
            {% set nested_result = [] %}
            {% for sub_item in item.sub_items %}
                {% if sub_item.condition %}  {# Corrected condition check #}
        ...
        {% endif %}
    {% endfor %}
    {% if result == [[12, 15, 19, 3], [13, 18]]%}
        {{ return('You win!  Macro is returning the right result! ' ~ result) }}
    {% else %}  
        {{ return(result) }}
    {% endif %}
{% endmacro %}