...
- Every mart model needs at least one model-level `data_tests` entry (a test declared under
  ...
  values against each other, for example:
  - `food_revenue + drink_revenue + other_revenue = total_revenue` (`location_performance.yml`)
  - `total_margin <= total_revenue` (`product_performance.yml`)
